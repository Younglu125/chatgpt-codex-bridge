...
The directly vendored third-party project is `codex-with-chatgpt`; this is not a claim that several
unidentified repositories were combined. Its source, MIT license and attribution are retained.
...
### Sharing with colleagues: full package or GitHub?

Both distribute the same program. **A complete package is not an already-connected Full setup.**
It contains Lite/Full source and the pinned upstream code, but is not an offline dependency bundle.
Accounts, authorizations, tunnels and preinstalled dependencies are not included. Each recipient
uses their own account and verifies their own machine/project.

- **Small initial trial:** send the reviewed release ZIP, especially for users without repository
  access or Git experience. Read its root `INSTALL.md`; plugin source is under `plugins/chatgpt-codex-bridge/`.
- **Ongoing use or maintenance:** prefer a GitHub clone for version tracking, updates and issue reports.
  A private repository needs access grants; a link alone does not grant access. GitHub's source ZIP
  and the marketplace ZIP generated by `scripts/release.py` have different layouts.
...
that proof. Routine Git updates: `git status`, `git pull --ff-only`, reinstall, then open a new task.
