...
发布状态：0.2.11 候选版。当前验收证据见 VALIDATION.md，不能把未验证的平台/账号写成已支持。

## 最简单的使用开关

- 新会话默认本地 Codex；没有开启 CCB 时，不自动转交 ChatGPT。
- 提示词出现上述任一标记：进入 CCB 模式，让普通 ChatGPT 加入当前任务。
- 标记只决定“ChatGPT 是否参与”，不要求同时写明分析、审查、复核、实现或修复阶段。
- Codex 动态判断参与时机：重要方案分析、大决策、独立复核优先考虑 ChatGPT；状态查询、编辑、测试和明确的小修复直接本地完成。权衡新增价值、等待时间和交接成本，不为省 token 牺牲整体效率。
- 明确说“这次必须让 GPT 协助”则本次必须调用；不可用时明确说明，不能把 Codex 的回答冒充 GPT 回复。
- 同一会话进入 CCB 后无需每轮重复标记，直到用户明确说退出 CCB 或只用本地 Codex。
- “这一步你直接处理”只覆盖本次，之后仍保持 CCB；新建其他会话默认 Codex。
- 否定、引用、代码文本或讨论/维护 CCB 本身不算开启。初始分析不会在每条消息重复；复核集中在有意义的阶段。

会话状态由 `scripts/session.py` 按实际 Codex 会话 ID 保存，工作区仍按项目独立绑定。
这是 Skill 驱动的规则，不是全局自动拦截器；Skill 未加载时存储文件本身不能强制宿主调用。
续接摘要需保留模式、会话 ID 和当前步骤；没有可靠会话 ID 时仅依赖会话上下文并明确说明。

例如，`CCB 帮我处理这个问题` 已足够触发。若只想分析，可以说 `CCB，只分析这个问题，不修改`；这些阶段描述是可选控制，不是触发 CCB 的必填格式。
...
路由器检查授权令牌、服务及已保存会话的一致性，但 `fullReady` 只是本地就绪判断，不能独立
证明 ChatGPT 已读到文件。复用前仍需遵循实际 `workspace_info` 和文件读取验证流程；
不能把保存了会话元数据或本地状态为绿，当作远端读取成功。
